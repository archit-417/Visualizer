...
function quick_partition (start, end)
{
    ...
    div_update(divs[start],div_sizes[start],c3);//Color update
        ...
        for(var j =start + 1; j <= end ; j++ )
        {
            ...
            if (div_sizes[ j ] < piv)
            {
                ...
                div_update(divs[i],div_sizes[i],c1);//Height update
                div_update(divs[j],div_sizes[j],c1);//Height update

                i += 1;
            }
    }
    div_update(divs[start],div_sizes[start],c2);//Color update
    ...
    div_update(divs[start],div_sizes[start],c2);//Height update
